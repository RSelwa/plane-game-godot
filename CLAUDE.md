...
- **`data/` — ALL CONTENT AS DATA.** Every file is GDScript with `static func`s, so it hot-reloads (F6, no restart, no rebuild):
  ...
  - `data/modules/<id>.gd` — **one file per module TYPE**, self-contained: scene, footprint, states, the facts it reads, its decision list. The three placeholder types (`switch`/`dial`/`lever`) were **deleted**; `airport_code` (the KTANE-style password module, model `password.glb`) is the only planned type and is being (re)built under the Model B contract below.
  ...
  - `data/campaign.gd` (`CockpitCampaign`) — **DELETED.** Was a hand-written mission list; superseded by seed-generated rounds (see "Round = parameters + a seed"). The registry (`defs()`) is currently empty until `airport_code` is registered.
...
- The required config is **DERIVED, never stored or shown** — only looked up. `manual_text()` renders each module as a numbered if/else list (the tower binder) — and renders EVERY registered type, not the round's (see "The manual is a BOOK" below).
- Authoring flow: `ModuleRegistry.build_manual_data(mission.modules)` (named constants → typo-safe) → `flight.gd` `JSON.stringify`s it → `brain.load_manual_json()` which **validates** (unknown fact/control/op/label = clear error) then derives. This payload feeds DERIVATION only — the tower's book is rendered separately.
...
### The manual is a BOOK, not a round summary (DECIDED, not built)
**The tower's manual is a fixed, complete book: one section per module TYPE, every registered type, always — never narrowed to the round's modules.** Rendering only the present modules leaks the dashboard: the tower opens its panel and already knows what the pilot has, so the pilot has nothing left to describe and half the information asymmetry dies. The tower must be TOLD which page to turn to, exactly as in KTANE.
- **Two consumers of the same rules, NOT two sources of truth.** The BOOK (`manual_text()`) renders every registered type and is stable from round to round — so a returning crew can *learn* it, which is what makes KTANE better on the tenth play. It costs nothing, because the content stays seed-rolled. The DERIVE payload (`build_manual_data`) stays per-round: the brain derives states only for controls actually registered.
- **A wheels module never joins the derive payload** — it has no decision list at all. Its book page is prose plus **the printed code pool** (the tower's lookup table). `build_manual_data()` skipping `kind == KIND_WHEELS` is permanent, not a workaround: keying the payload by module id would make `ManualEngine.load_modules()` reject an "unknown control", since a wheels module's controls are named `<module_id>/w<i>`.
- **Facts stay per-round** (the union of the present modules' facts). A fact is a physical placard in the cockpit — rolling one for an absent module would hang a placard that serves nothing. No conflict with the fixed book: the tower only reads the pages for the modules the pilot describes, and those modules' facts are rolled.
- Lands right after the `airport_code` wiring, together with the pool widening (6 → 25-40 codes) — the pool IS the `airport_code` page.

### Round = parameters + a seed (DECIDED, replaces the hand-authored mission list)
**A round is a set of PARAMETERS plus a SEED. The seed generates everything; the parameters only bound what it may generate.** There is no enumerated mission list — nothing anywhere names "mission m02". `data/campaign.gd` (`CockpitCampaign`) has been **deleted**; a generator (`data/mission_gen.gd`) replaces it. Not implemented yet.
...
- `state_match` (default) — generic: current state == the state derived from the decision list. A simple single-state-set module needs **zero code**; it is a data file plus a prefab.
...
### Module instances & per-instance edgework (Model B — DECIDED, not yet built)
A round picks module **INSTANCES**, not one-per-type. The same type may appear several times (`password_1`, `password_2`, …), so the brain keys by **instance id**, not type id — the per-instance suffix is the fix for the `control_store.gd:18` duplicate blocker.
- **Per-instance EDGEWORK.** Each instance rolls its own local content from the seed (a password's wheel count 3–6 and the letters on each wheel; a keypad's buttons). The instance's **state set is derived from its edgework** (3 buttons → 3 states, 6 → 6), so `state_labels` is no longer fixed per type — the spawner pushes the ROLLED states, not `def().states`.
- **Derivation context per instance = global facts ∪ that instance's edgework.** Same condition ops; a decision-list `when` may reference an edgework id exactly like a fact. This is what lets two instances of the SAME type land on different answers even though the flight facts (warning light, departure airport) are identical: their edgework differs.
- **The manual is FIXED and GENERIC per TYPE.** The conditions live in the manual itself (`if 3 buttons → …, else if 6 → …`). ONE block per type, never per instance. The pilot reads THEIR instance's edgework aloud; the tower applies the generic rule to those specific numbers. This is what preserves the KTANE information asymmetry when the same module appears several times — 4 identical instances would otherwise mean 4 redundant manual blocks and zero tension.
- **`max_instances` per type** caps how many of a type a round may draw (a genuinely-unique control = 1), declared in the module's own file.
- **Edgework generation is data-driven by default** (`gen:number`, pick-from-pool, pick-N-from-pool). A genuinely complex type (password letters with a uniqueness constraint, computed coordinates) declares a **named generation strategy in code** — the same rare/batched split as `state_match` vs `value_match`. Never per-module generation code for the simple cases.
...
- **The manual is a fixed BOOK, not a round summary.** One section per registered module TYPE, always all of them. Narrowing it to the round's modules told the tower what the pilot has — see "The manual is a BOOK" above.
- **`Mission` is a typed object, not a Dictionary** (`data/mission.gd`, `class_name Mission extends RefCounted`, `_init` requires every field). A Dictionary turned a typo'd key into a silently-wrong round, and a wrong puzzle looks exactly like a correct one. Same reason `MissionParams` was considered and dropped: one type, constructor-enforced, is enough.
...
- **Stripped to one module, Model B.** The three placeholder types (switch/dial/lever) and `data/campaign.gd` were **deleted** this session; only `airport_code` (password) survives, because it is the real final-version module. The pick model is now **Model B** (per-instance edgework, generic fixed per-type manual, instance ids — see "Module instances & per-instance edgework" above). The project is intentionally non-runnable until `airport_code` is registered under that contract.
...
### Milestones done
A′ data-layer + C# brain · B round loop (scenario→manual→timer→LAND→result) · C information-asymmetry puzzle · scene flow (menu/settings/recap) · decision-list manual engine (data-driven) · **module registry + mission/modes data layer** (`data/facts.gd`, `data/ops.gd`, `data/module_registry.gd`, `data/dashboard.gd`, `data/modes.gd` — data only). NOTE: the three placeholder module types and `data/campaign.gd` were since **deleted** (see "Stripped to one module, Model B"); the registry is now empty pending `airport_code`.
...
4. **Round generator** (`data/mission_gen.gd`): params + seed → round (`data/campaign.gd` already deleted). Picks module INSTANCES (duplicates allowed, `max_instances` respected). Then the lobby/free-play param screen (lives, time, module count) and the curated campaign seed list. Per-round recap with the full module breakdown.
